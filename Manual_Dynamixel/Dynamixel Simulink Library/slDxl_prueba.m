...
classdef slDxl < handle & matlab.mixin.SetGet & matlab.mixin.Copyable
    properties (Constant)
        ...
        DeviceName = 'myDxl';
        % Protocol version
        ProtocalVer            = '1.0';          % See which protocol version is used in the Dynamixel
        PROTOCOL_VERSION = 1;
        ...
        % Valores de la Tabla de Control AX-18A
        ...
        ADDR_MX_MOVING_SPEED = 32;                  % Dirección de Registro para Control de Velocidad
        ADDR_MX_PRESENT_POSITION = 36;              % Dirección de Registro Posición Actual
        ADDR_MX_PRESENT_SPEED = 38;                 % Dirección de Registro para Velocidad Actual
   ...
   methods       
       ...
       function [notfound, warnings] = initialize(myDxl)
          ...
          try
            port_num = calllib(lib_name, 'portHandler', DEVICENAME);
              ...
              defaultans = {hw.AvailableSerialPorts{1},'1000000'};
              ...
              if ~isempty(answer)
                 ...
                 error('Error: COM port is invalid');
              end
              port_num = calllib(lib_name, 'portHandler', DEVICENAME);
          end
      ...
      function setupManipulator(myDxl)
          myDxl.closePort();
          % Unload Library
